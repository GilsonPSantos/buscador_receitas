package br.com.buscadorReceitas.grupo.util;
...
public class ValidadorGrupo {
	...
	public void validar(Grupo grupo, String acao) throws ValidatorException, IOException, ValidationException {
		...
		try {
			...
			if(acao.equals("alterar")) {
				regra.setCampo(grupo.getClass().getDeclaredField("idGrupo"));
				regra.setCampoNulo(false);
				regra.setNomeCampo(Propriedade.getMessageProperties(propertiesCampos, ConstanteGrupo.NOME_CAMPO_ID_GRUPO));
			...
			regra = new Regra();
			regra.setCampo(grupo.getClass().getDeclaredField("nome"));
			regra.setCampoNulo(false);
			regra.setNomeCampo(Propriedade.getMessageProperties(propertiesCampos, ConstanteGrupo.NOME_CAMPO_NOME_GRUPO));
			...
			valida.validarCampos(grupo, listaRegras);
